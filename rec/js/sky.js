...
start()
{
	...
	for(var i=0; i<this.qtdeleaves; i++) {
		//console.table("start");
			...
			this.leave = new Leave(x,y, size,vel );
			//console.table(s.x);
			//console.table(this.leave);
		this.leaves[i] = this.leave;
	}
...
//console.table(leaves);
	...
	desenha()
	{
		...
		//desenha as folhas no plano de fundo
		this.ctx.fillStyle = '#fff';
		for(var i=0; i<this.leaves.length;i++) {
			this.leave = this.leaves[i];
			this.ctx.fillRect(this.leave.x, this.leave.y, this.leave.size, this.leave.size);
			//console.table(leave);
		}
  ...
  update(){
	...
	for(var i=0; i<this.leaves.length; i++) {
		...
		leave.y += dt * leave.velocity;
		//	If the leave has moved from the bottom of the screen, spawn it at the top.
		if(leave.y > this.height) {
			var x = Math.random()*this.width;
			var size =  Math.random()*6+3;
			var vel = (Math.random()*(this.maxVelocity - this.minVelocity))+this.minVelocity;
			let s = new Leave(x, 0,size, vel);
			//console.table(s);			
		 	this.leaves[i] = s;
		}
